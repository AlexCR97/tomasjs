...
export class UseControllers implements AppSetupFactory {
  ...
  private bootstrapController(
    ...
    const resolvedController = this.bootstrapToContainer(app, container, controller);
    this.bootstrapToHttpPipeline(app, resolvedController);
    this.logger?.debug(`Controller ${controller.name} has been successfully bootstrapped.`);
  }

  private bootstrapToContainer(
    app: Express,
    container: Container,
    controller: ClassConstructor<Controller>
  ) {
  ...
  private bootstrapToHttpPipeline(app: Express, controller: Controller) {
    ...
    const expressRouter = new ControllerAdapter({ controller, logger: this.logger }).adapt();
